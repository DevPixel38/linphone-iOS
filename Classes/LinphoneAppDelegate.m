...
-(NSDictionary *) toDictionary:(NSString *)strChangetoJSON{
    NSError *error;
...
-(void) registerWithService:(NSString *)deviceToken :(NSString *)callerId{

        NSMutableURLRequest *request = [[NSMutableURLRequest alloc]
                                        initWithURL:[NSURL
                                                     URLWithString:@"http://phone.antwork.com:8090/api/registerToken"]];
        
        [request setHTTPMethod:@"POST"];
        [request setValue:@"application/json"
       forHTTPHeaderField:@"Content-type"];
        
        NSString *jsonString = [NSString stringWithFormat: @"{\"callerId\": \"%@\", \"deviceToken\": \"%@\", \"serverCode\": 0}", callerId, deviceToken];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error{
    [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"isTokenSent"];
}

- (void)connection:(NSURLConnection *)connection didReceiveResponse:(nonnull NSURLResponse *)response{
    [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"isTokenSent"];
    NSString *token = [[NSString alloc] initWithData:[LinphoneManager.instance pushNotificationToken] encoding:NSUTF8StringEncoding];
...
- (void)processRemoteNotification:(NSDictionary *)userInfo {
    ...
    if (aps != nil) {
        NSDictionary *alert = [self toDictionary:[aps objectForKey:@"alert"]];
        if (alert != nil) {
...
didUpdatePushCredentials:(PKPushCredentials *)credentials
    ...
    BOOL isTokenSentToServer = [[NSUserDefaults standardUserDefaults] boolForKey:@"isTokenSent"];
    BOOL isUserLoggedIn = [[NSUserDefaults standardUserDefaults] boolForKey:@"IsLoggedIn"];
    
    if (isUserLoggedIn && !isTokenSentToServer){
        NSString *username = [[NSUserDefaults standardUserDefaults] valueForKey:@"CallerId"];
        [self registerWithService:token :username];
    }else{
        [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"isTokenSent"];
    }
