...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [store transformLinphoneCoreToKeys];
    NSString *test = [store objectForKey:@"account_mandatory_username_preference"];
    NSLog(@"%@", test);
    if (test != nil && ![test  isEqual: @""]){
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"IsLoggedIn"];
        [[NSUserDefaults standardUserDefaults] setValue:test forKey:@"CallerId"];
    }
    ...
    if (isLoggedIn){
        ...
        token = [token stringByReplacingOccurrencesOfString:@">" withString:@""];
        token = [token stringByReplacingOccurrencesOfString:@" " withString:@""];
        NSLog(@"%@", [LinphoneManager.instance pushNotificationToken]);
